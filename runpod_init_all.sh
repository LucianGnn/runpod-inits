...
#   # Vocile Morpheus (se pot schimba din env, dar defaulturile sunt din repo-ul tău):
#   VOICE_DIR_NAME      (default: indextts)
# Starts ComfyUI pe 0.0.0.0:8188
...
# 1) ComfyUI-Manager (ca să îl vezi în UI)
...
# 2) ComfyUI-Index-TTS (sursa completă a pachetului 'indextts' pentru importuri)
...
# 3) TTS-Audio-Suite (node-ul pe care îl folosești)
...
# Dependențe suplimentare pentru IndexTTS (evită erorile cu device_map/accelerate)
...
# 2) PYTHONPATH cu ambele surse (în cazul în care TTS-Audio-Suite folosește import absolut)
HF_ENV="$VENV/hf.env"
...
# Toate fișierele de voci (wav + .txt + .reference.txt) din datasetul tău
names = [
