...
# PIP cache persistent (accelerează re-booturile)
export PIP_DISABLE_PIP_VERSION_CHECK=1
export PIP_NO_INPUT=1
export PIP_DEFAULT_TIMEOUT=1000
export PIP_CACHE_DIR="$WORKSPACE/.cache/pip"
mkdir -p "$PIP_CACHE_DIR" "$WORKSPACE" "$WF_DST" "$VOICES_DST" "$INPUT_DIR"
...
# Torch (GPU dacă există). Cu cache persistent, re-booturile sunt rapide.
...
# Dependențe suplimentare (rezolvă tot ce ai în loguri)
python -m pip install -U \
  accelerate modelscope huggingface_hub hf_transfer \
  diffusers opencv-python-headless moviepy ffmpeg-python deepdiff \
  librosa==0.10.2.post1 soundfile numba pydub --progress-bar off || true
...
# 2) PYTHONPATH cu ambele surse (dacă TTS-Audio-Suite folosește import absolut)
HF_ENV="$VENV/hf.env"
...
# Voci Morpheus din dataset (wav + .txt + .reference.txt)
names = [
